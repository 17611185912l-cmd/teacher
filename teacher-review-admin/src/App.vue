...
interface Teacher { id:string; name:string; initials:string; level:string; type:string; phone:string; idNumber?:string; idCardFrontImageUrl?:string; idCardBackImageUrl?:string; educationCertificateImageUrls?:string[]; wechat:string; subjects:string[]; grades:string[]; tags:string[]; hours:number; rating:number; status:Status; online:boolean; auditStatus:AuditStatus; date:string; city:string; school:string; education:string; experience:number; intro:string; originalPortraitUrl?:string; aiPortraitUrl?:string; aiPortraitStatus?:'生成中'|'已生成' }
interface Query { name:string; wechat:string; phone:string; teacherType:string; onlineStatus:string; subject:string; auditStatus:string; range:string[] }
...
function cloneTeacher(teacher: Teacher): Teacher { return { ...teacher, subjects: [...teacher.subjects], grades: [...teacher.grades], tags: [...teacher.tags], educationCertificateImageUrls:[...(teacher.educationCertificateImageUrls || [])] } }
const queryRef = ref<FormInstance>()
const urlParams = new URLSearchParams(window.location.search)
const embedded = urlParams.get('embedded') === '1'
const contentTemplatePage = urlParams.get('view') === 'content-templates'
const query = reactive<Query>({ name:'',wechat:'',phone:'',teacherType:'',onlineStatus:'',subject:'',auditStatus:'',range:[] })
...
const rows = ref<Teacher[]>([
  ...
  {id:'900111',name:'万毅盈',initials:'万',level:'普通讲师',type:'骨干教员',phone:'130****9664',wechat:'-',subjects:['语文','数学','英语'],grades:['小学','初一-初二'],tags:['四六级'],hours:0,rating:0,status:'未授课',online:false,auditStatus:'待审核',date:'2026-06-18 14:32:18',city:'重庆市',school:'-',education:'本科',experience:3,intro:'专注小学学科辅导。',aiPortraitStatus:'生成中'},
...
const filtered = computed(() => rows.value.filter(r => (!query.name || r.name.includes(query.name)) && (!query.wechat || r.wechat.includes(query.wechat)) && (!query.phone || r.phone.includes(query.phone)) && (!query.teacherType || r.type === query.teacherType) && (!query.onlineStatus || (query.onlineStatus === '已上架' ? r.online : !r.online)) && (!query.subject || r.subjects.includes(query.subject)) && (!query.auditStatus || r.auditStatus === query.auditStatus)))
...
const defaultContentTemplates: ContentTemplateSettings = {
  ...
  achievements: [
    { id:'achievement-01', icon:'trend', title:'综合表现从中游进入班级前 10', subtitle:'持续提升 · 李同学', highlight:'进入前 10' },
    { id:'achievement-02', icon:'flag', title:'班级排名从 45 名 → 12 名', subtitle:'排名突破 · 张同学', highlight:'提升 33 名' },
    { id:'achievement-03', icon:'trend', title:'专项能力由基础级进阶到熟练级', subtitle:'能力进阶 · 王同学', highlight:'达到熟练级' },
    { id:'achievement-04', icon:'trend', title:'综合能力从 B 级提升至 A 级', subtitle:'阶段突破 · 陈同学', highlight:'提升至 A 级' },
    { id:'achievement-05', icon:'reading', title:'学习表现由待提升进入优秀档', subtitle:'稳步提升 · 周同学', highlight:'达到优秀档' },
    { id:'achievement-06', icon:'trend', title:'综合排名连续提升 28 名', subtitle:'方法突破 · 赵同学', highlight:'提升 28 名' },
    { id:'achievement-07', icon:'trend', title:'阶段评级由 C 级提升至 A 级', subtitle:'体系梳理 · 孙同学', highlight:'连升两级' },
    { id:'achievement-08', icon:'trophy', title:'单元表现进入班级前 5', subtitle:'基础巩固 · 刘同学', highlight:'进入前 5' },
    { id:'achievement-09', icon:'collection', title:'知识掌握能力从三级升至五级', subtitle:'知识积累 · 吴同学', highlight:'提升两级' },
    { id:'achievement-10', icon:'reading', title:'表达能力由基础档提升至优秀档', subtitle:'表达提升 · 郑同学', highlight:'达到优秀档' },
    { id:'achievement-11', icon:'aim', title:'基础训练正确率由合格提升至优秀', subtitle:'习惯改善 · 何同学', highlight:'达到优秀' },
    { id:'achievement-12', icon:'trend', title:'知识体系由基础级升至熟练级', subtitle:'知识梳理 · 林同学', highlight:'达到熟练级' },
    { id:'achievement-13', icon:'flag', title:'综合排名从 32 名 → 8 名', subtitle:'排名提升 · 胡同学', highlight:'提升 24 名' },
    { id:'achievement-14', icon:'notebook', title:'信息分析能力由合格档升至优秀档', subtitle:'分析能力 · 罗同学', highlight:'达到优秀档' },
    { id:'achievement-15', icon:'reading', title:'问题解决能力由 B 级提升至 A 级', subtitle:'方法规范 · 高同学', highlight:'提升至 A 级' },
    { id:'achievement-16', icon:'trophy', title:'综合竞赛首次参赛获省级二等奖', subtitle:'竞赛突破 · 徐同学', highlight:'省级二等奖' },
    { id:'achievement-17', icon:'aim', title:'任务完成准确度由合格进入优秀', subtitle:'准确度提升 · 马同学', highlight:'达到优秀' },
    { id:'achievement-18', icon:'timer', title:'作业完成效率由待提升进入优秀档', subtitle:'效率提升 · 梁同学', highlight:'达到优秀档' },
    { id:'achievement-19', icon:'trophy', title:'实践操作连续获评优秀', subtitle:'实践提升 · 许同学', highlight:'连续获评优秀' },
    { id:'achievement-20', icon:'trend', title:'综合排名从后段进入班级前 15', subtitle:'整体规划 · 唐同学', highlight:'进入前 15' }
  ]
}
const legacyAchievementPresetIds = new Set([
  'achievement-score', 'achievement-rank', 'achievement-geometry', 'achievement-gaokao-english', 'achievement-reading',
  'achievement-physics', 'achievement-chemistry', 'achievement-primary-math', 'achievement-primary-english', 'achievement-writing',
  'achievement-calculation', 'achievement-biology', 'achievement-geography', 'achievement-history', 'achievement-politics',
  'achievement-competition', 'achievement-english-reading', 'achievement-homework', 'achievement-experiment', 'achievement-total-score'
])
...
function normalizeAchievements(value: unknown): TeachingAchievementItem[] {
  const stored = Array.isArray(value) ? value.flatMap((item, index): TeachingAchievementItem[] => {
    if (!item || typeof item !== 'object') return []
    const candidate = item as Partial<TeachingAchievementItem>
    if (!candidate.title || !candidate.subtitle || !candidate.highlight) return []
    const icon = typeof candidate.icon === 'string' && (isBuiltInFocusIcon(candidate.icon) || isUploadedFocusIcon(candidate.icon)) ? candidate.icon : 'trend'
    return [{ id:candidate.id || `achievement-${index}-${Date.now()}`, icon, title:candidate.title, subtitle:candidate.subtitle, highlight:candidate.highlight }]
  }) : []
  const presetIds = new Set([...defaultContentTemplates.achievements.map(item => item.id), ...legacyAchievementPresetIds])
  const customItems = stored.filter(item => !presetIds.has(item.id))
  return [...defaultContentTemplates.achievements.map(item => ({ ...item })), ...customItems]
}
...
const inlineAchievementDraft = reactive<TeachingAchievementItem>({ id:'', icon:'', title:'', subtitle:'', highlight:'' })
const achievementTableRows = computed<TeachingAchievementItem[]>(() => editingAchievementId.value === newAchievementRowId
  ? [...teachingAchievements.value, { id:newAchievementRowId, icon:'', title:'', subtitle:'', highlight:'' }]
...
async function saveTeacherTag(): Promise<void> {
  ...
  if (name.length < 2 || name.length > 6) { ElMessage.warning('标签名称长度为 2 至 6 个字符'); return }
...
async function deleteTeacherTag(tag: TeacherTag): Promise<void> {
  try {
    await ElMessageBox.confirm(`确定删除教师标签“${tag.name}”吗？`, '删除教师标签', { type:'warning', confirmButtonText:'删除', cancelButtonText:'取消' })
  } catch { return }
...
let pendingPortraitTeacherId: string | null = null
...
function startPortraitGeneration(file: UploadFile): void {
  ...
  reader.onload = () => {
    ...
    const teacherId = current.value.id
    const generatedPortraitUrl = reader.result
    originalPortraitUrl.value = reader.result
    current.value.originalPortraitUrl = reader.result
    current.value.aiPortraitStatus = '生成中'
    pendingPortraitTeacherId = teacherId
    portraitGenerating.value = true
    window.setTimeout(() => {
      if (pendingPortraitTeacherId !== teacherId) return
      // 原型阶段以本地上传图模拟 AI 成片，接入服务后替换为生成结果 URL。
      const row = rows.value.find(item => item.id === teacherId)
      if (row?.aiPortraitStatus === '生成中') {
        row.aiPortraitUrl = generatedPortraitUrl
        row.aiPortraitStatus = '已生成'
      }
      if (current.value.id === teacherId) {
        aiPortraitUrl.value = generatedPortraitUrl
        current.value.aiPortraitUrl = generatedPortraitUrl
        current.value.aiPortraitStatus = '已生成'
        portraitGenerating.value = false
      }
      pendingPortraitTeacherId = null
      ElMessage.success('AI 形象照已生成')
    }, 1800)
  }
  reader.readAsDataURL(file.raw)
}
type IdCardImageField = 'idCardFrontImageUrl' | 'idCardBackImageUrl'
function validateQualificationImage(label: string, file: UploadFile): file is UploadFile & { raw: File } {
  if (!file.raw) return false
  const supportedTypes = ['image/png', 'image/jpeg', 'image/webp']
  if (!supportedTypes.includes(file.raw.type)) {
    ElMessage.warning(`${label}仅支持 JPG、PNG 或 WebP 图片`)
    return false
  }
  if (file.raw.size > 10 * 1024 * 1024) {
    ElMessage.warning(`${label}图片大小不能超过 10MB`)
    return false
  }
  return true
}
function handleIdCardImageUpload(field: IdCardImageField, label: string, file: UploadFile): void {
  if (!validateQualificationImage(label, file)) return
  const reader = new FileReader()
  reader.onload = () => {
    if (typeof reader.result !== 'string') return
    current.value[field] = reader.result
    ElMessage.success(`${label}已上传`)
  }
  reader.readAsDataURL(file.raw)
}
function handleIdCardFrontImageUpload(file: UploadFile): void {
  handleIdCardImageUpload('idCardFrontImageUrl', '身份证人像面', file)
}
function handleIdCardBackImageUpload(file: UploadFile): void {
  handleIdCardImageUpload('idCardBackImageUrl', '身份证国徽面', file)
}
function handleEducationCertificateImageUpload(file: UploadFile): void {
  if (!validateQualificationImage('学历证明', file)) return
  const reader = new FileReader()
  reader.onload = () => {
    if (typeof reader.result !== 'string') return
    current.value.educationCertificateImageUrls = [...(current.value.educationCertificateImageUrls || []), reader.result]
    ElMessage.success('学历证明已上传')
  }
  reader.readAsDataURL(file.raw)
}
function removeIdCardImage(field: IdCardImageField, label: string): void {
  current.value[field] = undefined
  ElMessage.success(`${label}已移除，保存后生效`)
}
function removeEducationCertificateImage(index: number): void {
  current.value.educationCertificateImageUrls = (current.value.educationCertificateImageUrls || []).filter((_, imageIndex) => imageIndex !== index)
  const label = '学历证明'
  ElMessage.success(`${label}图片已移除，保存后生效`)
}
const drawer = ref(false); const editorDialog = ref(false); const active = ref<'base'|'display'>('base'); const current = ref<Teacher>(cloneTeacher(rows.value[0])); const saving = ref(false); const page = ref(1); const editing = ref(false); const auditDialog = ref(false); const auditDecision = ref<AuditStatus>('审核通过'); const auditNote = ref('')
const isPortraitGenerating = computed(() => portraitGenerating.value || current.value.aiPortraitStatus === '生成中')
...
function startNewAchievement(): void {
  if (!ensureAchievementEditorAvailable()) return
  Object.assign(inlineAchievementDraft, { id:'', icon:'', title:'', subtitle:'', highlight:'' })
...
async function saveAchievementItem(): Promise<void> {
  ...
  await new Promise<void>(resolve => window.setTimeout(resolve, 300))
  const nextItem: TeachingAchievementItem = { id:inlineAchievementDraft.id || `achievement-${Date.now()}`, icon:inlineAchievementDraft.icon, title, subtitle, highlight }
...
function open(row:Teacher, display=false, edit=false):void { portraitGenerating.value=false; current.value=cloneTeacher(row); setPortraitUrls(current.value); active.value=display?'display':'base'; editing.value=edit; editorDialog.value=true }
function beginTeacherEdit():void { editing.value=true }
function cancelTeacherEdit():void {
  if (pendingPortraitTeacherId === current.value.id) pendingPortraitTeacherId = null
...
async function save():Promise<void>{saving.value=true; await new Promise<void>(r=>window.setTimeout(r,450)); const index=rows.value.findIndex(row=>row.id===current.value.id);if(index>=0)rows.value[index]=cloneTeacher(current.value);saving.value=false;editorDialog.value=false;ElMessage.success('教师资料已保存')}
function openAudit(): void {
  if (isPortraitGenerating.value) {
    ElMessage.warning('AI 形象照生成中，请等待生成完成后再审核')
    return
  }
  auditDialog.value = true
}
async function submitAudit():Promise<void>{if(isPortraitGenerating.value&&auditDecision.value==='审核通过'){ElMessage.warning('AI 形象照生成中，暂不能审核通过');return};if(auditDecision.value==='已驳回'&&!auditNote.value.trim()){ElMessage.warning('请填写驳回原因');return};saving.value=true;await new Promise<void>(r=>window.setTimeout(r,450));current.value.auditStatus=auditDecision.value;const index=rows.value.findIndex(row=>row.id===current.value.id);if(index>=0)rows.value[index]=cloneTeacher(current.value);saving.value=false;auditDialog.value=false;editorDialog.value=false;auditNote.value='';ElMessage.success(auditDecision.value==='审核通过'?'审核已通过':'已驳回教师申请')}
</script>
  ...
  <section class="main"><header class="top"><el-icon><Menu/></el-icon><el-icon><Refresh/></el-icon><span>▣ 教务管理　›　{{ contentTemplatePage ? '教师内容管理' : '教师管理' }}</span><div class="top-right"><el-button link type="primary" :icon="SwitchButton" @click="goToOnboarding">教师端入驻</el-button><el-icon><Bell/></el-icon><el-avatar :size="28">管</el-avatar></div></header><div class="tabs"><span>▣ 分析页　×</span><span>✥ 用户团队　×</span><span>▣ 课程管理　×</span><b>● {{ contentTemplatePage ? '教师内容管理' : '教师管理' }}　×</b></div>
    <main v-if="contentTemplatePage" class="content template-management">
      ...
      <section class="template-section"><div class="template-section-head"><p>供老师选择后带入简介，带入后仍可编辑。</p><div class="template-section-actions"><span>共 {{ introTemplates.length }} 条</span><el-button size="small" :icon="Plus" @click="startNewInlineTemplate">新增简介模板</el-button></div></div>
        <el-table :data="templateTableRows" border stripe row-key="id" empty-text="暂无个人简介模板" :row-class-name="templateRowClassName">
          ...
          <el-table-column prop="updatedAt" label="最近更新" width="160"/>
              <el-table-column label="状态" width="100"><template #default="{row}"><el-switch v-if="editingTemplateId === row.id" v-model="inlineTemplateDraft.enabled" inline-prompt active-text="启用" inactive-text="停用"/><el-switch v-else v-model="row.enabled" inline-prompt active-text="启用" inactive-text="停用" :disabled="editingTemplateId !== null" @change="updateIntroTemplateStatus(row)"/></template></el-table-column>
      ...
      <section class="template-section achievement-template-section">
        <div class="template-section-head"><p>维护用户端教师主页展示的教学成果案例，仅在教师内容管理中配置。</p><div class="template-section-actions"><span>共 {{ teachingAchievements.length }} 条</span><el-button size="small" :icon="Plus" @click="startNewAchievement">新增教学成果</el-button></div></div>
        <el-table :data="achievementTableRows" border stripe row-key="id" empty-text="暂无教学成果" :row-class-name="achievementRowClassName">
          ...
          <el-table-column label="主标题" min-width="250" show-overflow-tooltip><template #default="{row}"><el-input v-if="editingAchievementId === row.id" v-model="inlineAchievementDraft.title" maxlength="50" show-word-limit placeholder="例如：班级排名从 45 名 → 12 名"/><b v-else>{{ row.title }}</b></template></el-table-column>
          <el-table-column label="副标题" width="210"><template #default="{row}"><el-input v-if="editingAchievementId === row.id" v-model="inlineAchievementDraft.subtitle" maxlength="30" show-word-limit placeholder="例如：排名突破 · 李同学"/><span v-else>{{ row.subtitle }}</span></template></el-table-column>
          <el-table-column label="强化副标题" width="180"><template #default="{row}"><el-input v-if="editingAchievementId === row.id" v-model="inlineAchievementDraft.highlight" maxlength="20" show-word-limit placeholder="例如：提升 33 名"/><strong v-else class="achievement-highlight">{{ row.highlight }}</strong></template></el-table-column>
    ...
    <div class="editor-layout">
      ...
      <el-scrollbar class="editor-body">
        <el-form :model="current" label-position="top" class="editor-form" :disabled="!editing">
          ...
          <section class="form-section"><h3>教学能力配置</h3><el-form-item label="授课年级" required><el-checkbox-group v-model="current.grades" :disabled="!editing" @change="handleGradesChange"><el-checkbox v-for="grade in ['小学','初一-初二','初三','高一-高二','高三']" :key="grade" :label="grade"/></el-checkbox-group></el-form-item><el-form-item label="授课科目" required><el-checkbox-group v-model="current.subjects" :disabled="!editing"><el-checkbox v-for="subject in subjectOptions" :key="subject" :label="subject" :disabled="!editing || isSubjectDisabled(subject)"/></el-checkbox-group></el-form-item><el-row :gutter="18"><el-col :span="8"><el-form-item label="教龄（年）" required><el-input v-model.number="current.experience" class="numeric-input" type="number" inputmode="numeric" :disabled="!editing" min="0" max="99" step="1"/></el-form-item></el-col><el-col :span="16"><el-form-item label="授课方式"><el-checkbox :model-value="true" :disabled="!editing">线上</el-checkbox><el-checkbox :disabled="!editing">线下</el-checkbox><el-checkbox :model-value="true" :disabled="!editing">上门</el-checkbox></el-form-item></el-col></el-row></section>
          <section class="form-section"><h3>学历与资格认证</h3><el-row :gutter="18"><el-col :span="8"><el-form-item label="最高学历" required><el-select v-model="current.education" :disabled="!editing"><el-option value="本科"/><el-option value="硕士研究生"/><el-option value="博士研究生"/></el-select></el-form-item></el-col><el-col :span="8"><el-form-item label="毕业院校" required><el-input v-model="current.school" :disabled="!editing"/></el-form-item></el-col><el-col :span="8"><el-form-item label="学历展示"><el-switch :model-value="true" :disabled="!editing" inline-prompt active-text="展示" inactive-text="隐藏"/></el-form-item></el-col></el-row><el-row :gutter="18"><el-col :span="8"><el-form-item label="身份证号"><el-input v-model="current.idNumber" maxlength="18" placeholder="请输入身份证号" :disabled="!editing"/></el-form-item></el-col></el-row><el-row :gutter="18"><el-col :span="12"><el-form-item label="身份证"><div class="qualification-image-field"><div class="id-card-side-grid"><div class="id-card-side"><strong>人像面</strong><div class="qualification-image-item"><el-image v-if="current.idCardFrontImageUrl" class="qualification-image-preview" :src="current.idCardFrontImageUrl" :preview-src-list="[current.idCardFrontImageUrl]" :preview-teleported="true" fit="cover" alt="身份证人像面"/><div v-else class="qualification-image-empty">暂无图片</div><el-button v-if="editing && current.idCardFrontImageUrl" class="qualification-image-remove" circle size="small" type="danger" aria-label="删除身份证人像面" @click="removeIdCardImage('idCardFrontImageUrl', '身份证人像面')"><CircleClose/></el-button></div><el-upload v-if="editing" accept="image/png,image/jpeg,image/webp" :auto-upload="false" :show-file-list="false" @change="handleIdCardFrontImageUpload"><el-button size="small" :icon="UploadFilled">{{ current.idCardFrontImageUrl ? '重新上传' : '上传人像面' }}</el-button></el-upload></div><div class="id-card-side"><strong>国徽面</strong><div class="qualification-image-item"><el-image v-if="current.idCardBackImageUrl" class="qualification-image-preview" :src="current.idCardBackImageUrl" :preview-src-list="[current.idCardBackImageUrl]" :preview-teleported="true" fit="cover" alt="身份证国徽面"/><div v-else class="qualification-image-empty">暂无图片</div><el-button v-if="editing && current.idCardBackImageUrl" class="qualification-image-remove" circle size="small" type="danger" aria-label="删除身份证国徽面" @click="removeIdCardImage('idCardBackImageUrl', '身份证国徽面')"><CircleClose/></el-button></div><el-upload v-if="editing" accept="image/png,image/jpeg,image/webp" :auto-upload="false" :show-file-list="false" @change="handleIdCardBackImageUpload"><el-button size="small" :icon="UploadFilled">{{ current.idCardBackImageUrl ? '重新上传' : '上传国徽面' }}</el-button></el-upload></div></div><small v-if="editing">人像面、国徽面各 1 张；JPG/PNG/WebP，单张不超过 10MB</small></div></el-form-item></el-col><el-col :span="12"><el-form-item label="学历证明"><div class="qualification-image-field"><div v-if="current.educationCertificateImageUrls?.length" class="qualification-image-grid"><div v-for="(imageUrl,index) in current.educationCertificateImageUrls" :key="`${imageUrl}-${index}`" class="qualification-image-item"><el-image class="qualification-image-preview" :src="imageUrl" :preview-src-list="current.educationCertificateImageUrls" :initial-index="index" :preview-teleported="true" fit="cover" :alt="`学历证明图片${index + 1}`"/><el-button v-if="editing" class="qualification-image-remove" circle size="small" type="danger" aria-label="删除学历证明图片" @click="removeEducationCertificateImage(index)"><CircleClose/></el-button></div></div><div v-else class="qualification-image-empty">暂无图片</div><div v-if="editing" class="qualification-image-actions"><el-upload multiple accept="image/png,image/jpeg,image/webp" :auto-upload="false" :show-file-list="false" @change="handleEducationCertificateImageUpload"><el-button size="small" :icon="UploadFilled">{{ current.educationCertificateImageUrls?.length ? '继续上传' : '上传图片' }}</el-button></el-upload></div><small v-if="editing">支持多张；JPG/PNG/WebP，单张不超过 10MB</small></div></el-form-item></el-col></el-row></section>
...
.template-management .el-switch,.list .el-switch,.editor-form .el-switch{--el-switch-on-color:#2f7df6;--el-switch-off-color:#d7dce5}.template-management .el-switch .el-switch__core,.editor-form .el-switch .el-switch__core{min-width:48px!important;height:22px}.list .el-switch .el-switch__core{min-width:64px!important;height:22px}.template-management .el-switch .el-switch__action,.list .el-switch .el-switch__action,.editor-form .el-switch .el-switch__action{width:16px;height:16px}.template-management .el-switch .el-switch__inner .is-text,.list .el-switch .el-switch__inner .is-text,.editor-form .el-switch .el-switch__inner .is-text{font-size:12px;font-weight:600;letter-spacing:0}
...
.qualification-image-field{display:grid;gap:8px;min-height:94px}.qualification-image-grid{display:grid;grid-template-columns:repeat(3,104px);gap:10px}.id-card-side-grid{display:grid;grid-template-columns:repeat(2,104px);gap:16px}.id-card-side{display:grid;gap:7px}.id-card-side>strong{font-size:12px;font-weight:500;color:#606b7a}.qualification-image-item{position:relative;width:104px;height:76px}.qualification-image-preview,.qualification-image-empty{width:104px;height:76px;border:1px solid #dfe6ee;border-radius:4px;overflow:hidden}.qualification-image-empty{display:flex;align-items:center;justify-content:center;border-style:dashed;background:#fafbfd;color:#9aa5b1;font-size:12px}.qualification-image-remove{position:absolute;top:-7px;right:-7px;width:22px!important;height:22px!important;padding:0!important}.qualification-image-remove svg{width:13px;height:13px}.qualification-image-actions{display:flex;align-items:center;gap:10px}.qualification-image-field>small{color:#8a94a3;font-size:11px;line-height:18px}.qualification-image-actions .el-button,.id-card-side .el-button{margin-left:0;width:104px}
</style>
...
.editor-body{box-sizing:border-box;height:100%;padding:18px 26px 24px}.editor-form{max-width:850px}.form-section{padding:0 0 16px;margin-bottom:16px;border-bottom:1px solid #ebeff4}.form-section:last-child{border-bottom:0}.form-section h3{margin:0 0 14px;font-size:14px;color:#2f3a4b}.editor-form .el-form-item{margin-bottom:14px}.editor-form .el-form-item__label{padding-bottom:5px;font-size:12px;line-height:18px;color:#606b7a}.editor-form .el-select,.editor-form .el-input-number{width:100%}.editor-form .el-checkbox{margin-right:18px;font-size:12px}.editor-form .el-tag{margin-right:5px}.teacher-focus-options{display:grid;grid-template-columns:repeat(3,minmax(0,1fr));gap:10px;width:100%}.editor-form .teacher-focus-options .el-checkbox{position:relative;align-items:stretch;width:100%;height:auto;min-height:92px;margin:0;padding:12px;border:1px solid #dfe6ee;border-radius:5px;background:#fff;white-space:normal}.teacher-focus-options .el-checkbox__input{position:absolute;opacity:0;pointer-events:none}.teacher-focus-options .el-checkbox__label{width:100%;padding:0;white-space:normal}.teacher-focus-options .el-checkbox.is-checked{border-color:#409eff;background:#f3f8ff}.teacher-focus-options .el-checkbox.is-disabled{opacity:1}.teacher-focus-options .el-checkbox.is-disabled .el-checkbox__label{color:inherit}.teacher-focus-card{display:flex;align-items:flex-start;gap:10px}.teacher-focus-card>.el-icon,.teacher-focus-card>img{flex:0 0 25px;width:25px;height:25px;margin-top:1px}.teacher-focus-card>.el-icon{color:#1677ff;font-size:25px}.teacher-focus-card>img{object-fit:contain}.teacher-focus-card b,.teacher-focus-card small{display:block;letter-spacing:0}.teacher-focus-card b{margin-bottom:5px;color:#2f3a4b;font-size:13px;line-height:18px}.teacher-focus-card small{color:#7b8796;font-size:11px;line-height:17px}.numeric-input input::-webkit-outer-spin-button,.numeric-input input::-webkit-inner-spin-button{-webkit-appearance:none;margin:0}.numeric-input input[type=number]{appearance:textfield;-moz-appearance:textfield}.portrait-upload{display:flex;align-items:flex-start;flex-direction:column;gap:10px;min-height:104px}.portrait-pair{display:flex;gap:10px}.portrait-item{display:grid;gap:5px;min-width:0}.portrait-item>span{color:#606b7a;font-size:12px;line-height:18px;text-align:center}.portrait-preview{display:block;width:104px;height:78px;border:1px solid #dfe6ee;border-radius:4px;overflow:hidden}.portrait-generating{display:flex;align-items:center;justify-content:center;gap:5px;width:104px;height:78px;border:1px dashed #99c7ec;border-radius:4px;background:#f5faff;color:#4383b8;font-size:12px}.portrait-generating .el-icon{font-size:16px}.portrait-upload .el-button{padding:0}@media(max-width:760px){.portrait-pair{flex-wrap:wrap}}
.audit-machine{margin-top:16px;padding:18px 20px;border:1px solid #dfe5ec;background:#fff}.audit-machine-head{display:flex;align-items:flex-start;justify-content:space-between;gap:20px;padding-bottom:14px;border-bottom:1px solid #edf0f4}.audit-machine-head h3{margin:0 0 5px;color:#263246;font-size:15px}.audit-machine-head p{margin:0;color:#7b8796;font-size:12px}.audit-machine-head>span{color:#7b8796;font-size:12px;white-space:nowrap}.audit-machine-flow{display:grid;grid-template-columns:180px 1fr;gap:42px;align-items:center;padding:18px 0}.audit-state{display:grid;gap:5px;place-items:center;min-height:68px;padding:10px 16px;border:1px solid #e6b75b;background:#fff9eb}.audit-state small{color:#8a6a2b;font-size:11px}.audit-state strong{color:#8a5a00;font-size:15px}.audit-paths{display:grid;gap:10px}.audit-path{display:grid;grid-template-columns:80px 20px 110px 1fr;gap:10px;align-items:center;min-height:36px}.audit-path span,.audit-path em{font-size:12px;font-style:normal}.audit-path span{color:#596575}.audit-path i{color:#98a3b2;font-style:normal;text-align:center}.audit-path strong{padding:7px 12px;border-radius:3px;font-size:13px;text-align:center}.audit-path em{color:#7b8796}.audit-state-success{border:1px solid #9cd7b0;background:#f0faf3;color:#287a44}.audit-state-rejected{border:1px solid #edb0b0;background:#fff4f4;color:#b23a3a}.audit-machine-guard{display:flex;gap:14px;align-items:flex-start;padding:11px 13px;border-left:3px solid #409eff;background:#f4f8fd}.audit-machine-guard b{flex:0 0 auto;color:#2d5f96;font-size:12px}.audit-machine-guard span{color:#536174;font-size:12px;line-height:18px}@media(max-width:900px){.audit-machine-head{display:block}.audit-machine-head>span{display:block;margin-top:8px}.audit-machine-flow{grid-template-columns:1fr;gap:12px}.audit-path{grid-template-columns:70px 18px 100px 1fr}}
